  ...
  <div class="app-container">
      ...
      <el-tabs value="add" @tab-click="handleClick">
        <el-tab-pane name="add" label="添加">
          ...
          >
            ...
            <el-form-item label="Rank">
              ...
              &nbsp;&nbsp;&nbsp;
              <el-tooltip
                content="默认分数为2.5分，可根据漏洞的利用难度进行评判"
                placement="top"
              >
            ...
            <el-form-item label="描述">
              <el-input
                type="textarea"
                v-model="vulInfo.desc"
                size="medium"
              ></el-input>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" @click="uploadImg" size="medium"
                >提 交</el-button
              >
            </el-form-item>
          </el-form>
        ...
        <el-tab-pane name="local" label="本地导入">
          <div class="filter-container">
            ...
            >&nbsp;&nbsp;&nbsp;
            <el-tooltip
              content="一键导入默认导入分数为 2.5 分,漏洞名称为镜像名称,漏洞描述为漏洞名称"
              placement="top"
            >
              <i class="el-icon-question"></i>
            </el-tooltip>
          ...
          <el-table
            :data="
              localImageList.filter(
                (data) =>
                  !localSearch ||
                  data.name.toLowerCase().includes(localSearch.toLowerCase())
              )
            "
            @selection-change="handleSelectLocalImages"
            tooltip-effect="dark"
            style="width: 100%"
          ...
          >
            ...
            <el-table-column label="标签" width="120">
              <template slot-scope="{ row }">
                <el-tag v-if="row.flag === true" effect="dark" type="info"
                  >已导入</el-tag
                >
                <el-tag v-else-if="row.flag === false" effect="dark"
                  >未导入</el-tag
                >
              </template>
            </el-table-column>
            <el-table-column fixed="right" label="操作" width="120">
              <template slot-scope="{ row }">
                <el-button
                  @click.native.prevent="handleLocalRemove(row.name)"
                  type="danger"
                  size="small"
                  >移除</el-button
                >
    ...
    >
      <div v-loading="progressLoading">
        <el-row
          v-for="(item, index) in progress.layer"
          style="margin-bottom: 10px; height: 24px"
        >
          <el-tag
            style="float: left; width: 15%; height: 24px; line-height: 24px"
            align="center"
            >{{ item.id }}</el-tag
          >
    ...
    <el-dialog :visible.sync="editShow" title="修改">
      ...
      >
        <el-form-item label="漏洞名称">
          <el-input
            v-model="editVulInfo.image_vul_name"
            size="medium"
          ></el-input>
        </el-form-item>
        <el-form-item label="镜像">
        ...
        <el-form-item label="描述">
          <el-input
            type="textarea"
            v-model="editVulInfo.image_desc"
            size="medium"
          ></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="handleEditImage" size="medium"
            >提 交</el-button
          >
        </el-form-item>
      </el-form>
        ...
        <template slot-scope="{ row }">
          ...
          <el-tag
            style="
              ...
              margin-left: 5px;
            "
            @click="openProgress(row, 1)"
            effect="dark"
            v-if="row.is_ok === false && row.status.task_id !== ''"
          >
            ...
            <div style="display: inline-block; float: left">
              <el-progress
                ...
                :show-text="false"
                :text-inside="false"
                :percentage="row.status.progress"
                :width="20"
              ></el-progress>
            </div>
          </el-tag>
          ...
          <el-tag
            ...
            type="success"
            effect="dark"
            v-if="row.is_ok === true && row.is_share === true"
          >
            <div style="display: inline-block; float: left">
              <span>已分享</span>
            </div>
          ...
          <el-tag
            style="
              ...
              margin-left: 5px;
            "
            @click="openProgress(row, 2)"
            effect="dark"
            v-if="row.is_ok === true && row.status.progress_status === 'share'"
          >
            <div style="display: inline-block; float: left">
              <span>分享中</span>
            </div>
...
export default {
  ...
  methods: {
    ...
    initTableData() {
      ...
      ImgList(undefined, true, this.curentPage).then((response) => {
            ...
            if (response.data.data != null && response.data.status === 200) {
              ...
              if (
                ...
                (this.progress.count !== 0 &&
                  ...
                  this.progress.count === this.progress.total)
              ) {
    ...
    handleEditImage() {
      ...
      ImageEdit(this.editVulInfo.image_id, this.editVulInfo).then(
        (response) => {
